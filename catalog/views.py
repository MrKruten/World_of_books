...
def index(request):
    ...
    # Книги в заказе (статус= 'В заказе')
    num_instances_available = BookInstance.objects.filter(status__exact=2).count()

    # Авторы книг
    num_authors = Author.objects.count()

    # Кол-во посещений этого view, подсчитанного в сессии
...
class LoanedBooksByUserListView(LoginRequiredMixin, generic.ListView):
    # Универсальный класс представленияспискакниг,находящихся
    # в заказе у текущего пользователя.
    model = BookInstance
